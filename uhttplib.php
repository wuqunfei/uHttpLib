...
class UHttpLib
{
    ...
    function __construct($url, $keepalive = false, $timeout = 60)
    {
        $this->SetUrl($url);
    ...
    function BasicHeader($method, $params, $headerarray)
    {
        ...
        if (!array_key_exists('Host', $headerarray)) {
    ...
    function Get($paramarray = array(), $details = false, $headerarray = array())
    {
        ...
        $headers = array_merge($headerarray, $this->setCookie());
        $req = $this->BasicHeader(UHttpLib::GET, $this->RequestEncode($paramarray), $headers);
    ...
    function Put($paramarray = array(), $details = false, $headerarray = array())
    {
        ...
        $headers = array_merge($headerarray, $this->setCookie());
        $req = $this->BasicHeader(UHttpLib::PUT, '', $headers) . $content;
        return $this->Request($req, $details);
    }
    ...
    function Request($req, $details = false)
    {
        ...
        // My Cookie is important
        if (is_array($headers)) {
            $this->getCookie($headers);
        }

        //Redirection?
        if (isset($headers['location'])) {
            $http = new UHttpLib($headers['location']);
            return $http->Request($req, $details);
        } else {
            if ($details) {
                $result[UHttpLib::HTTP] = $code;
                $result[UHttpLib::HEADER] = $headers;
                $result[UHttpLib::BODY] = $body;
                return $result;
            } else {
                return $body;
            }
    ...
    function setCookie()
    {
        ...
        if (is_array($this->cookies) && count($this->cookies) > 0) {
            ...
            foreach ($this->cookies as $key => $value) {
                $cookieLineAll .= $key . "=" . $value . "; ";
            }
            if (strlen($cookieLineAll) > 2) {
                $cookieLine = substr($cookieLineAll, 0, strlen($cookieLineAll) - 2);
                $cookie['Cookie'] = $cookieLine;
            }
    ...
    function tree($str, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT)
    {
